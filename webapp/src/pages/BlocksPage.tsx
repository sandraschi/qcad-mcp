...
export default function BlocksPage() {
  ...
  useEffect(() => {
    fetch("/api/v1/blocks/categories").then(r => r.json()).then(j => {
  ...
  const downloadBlock = async (item: BlockResult) => {
    ...
    try {
      const r = await fetch("/api/v1/blocks/download", {
        ...
        body: JSON.stringify({ title: item.title, source: item.source, url: item.url }),
      });
      const j = await r.json();
      if (j.success) { setRecent(prev => [j.filename, ...prev].slice(0, 5)); setError(""); }
