...
export default function ViewerPage() {
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      {layers.length > 0 && (
        <div className="bg-[#0f0f12] border border-white/5 rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <button onClick={refreshSvg} className="ml-auto flex items-center gap-1 px-3 py-1 rounded-lg bg-white/5 text-xs text-slate-400 hover:text-white">
            ...
            {layers.map((l) => (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                  enabledLayers.has(l) ? "bg-amber-600 text-white" : "bg-white/5 text-slate-500"
                }`}
              >
      ...
      {svgUrl && (
        <div className="bg-[#0f0f12] border border-white/5 rounded-2xl p-4 overflow-auto" style={{ maxHeight: "70vh" }}>
      ...
      {info && (
        <div className="bg-[#0f0f12] border border-white/5 rounded-2xl p-4 space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2 text-sm">
            <div className="text-slate-500">Entities: <span className="text-slate-300">{info.entity_total}</span></div>
            <div className="text-slate-500">Layers: <span className="text-slate-300">{info.layer_count}</span></div>
            <div className="text-slate-500">Blocks: <span className="text-slate-300">{info.block_count}</span></div>
            <div className="text-slate-500">DXF Version: <span className="text-slate-300">{info.dxf_version}</span></div>
            {info.bounding_box && (
