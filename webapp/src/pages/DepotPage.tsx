...
const ENTITY_COLORS: Record<EntityType, string> = {
	line: "text-blue-400",
	rect: "text-emerald-400",
	circle: "text-amber-400",
	text: "text-indigo-400",
	polyline: "text-rose-400",
};
...
interface PlanInfoResult {
	...
	layers: Array<{ name: string; color: number; frozen?: boolean }>;
}
...
export default function DepotPage() {
	...
	const [fileInfo, setFileInfo] = useState<PlanInfoResult | null>(null);
		...
		try {
			const svgR = await fetch("/api/v1/control/tool", {
				...
				body: JSON.stringify({
					...
					arguments: {
						file_name: name,
						output_name: `_preview_${name}.svg`,
						background: "#0a0a0c",
					},
				}),
			});
	...
	const handleCreate = async () => {
		...
		try {
			...
			const layers = uniqueLayers.map((name, i) => ({
				name,
			...
			setCreateName("");
			setCreateDesc("");
			setCreateEntities([{ type: "rect", x: 0, y: 0, w: 10000, h: 8000, layer: "walls" }]);
			await loadFiles();
		} catch (err: unknown) {
	...
	const handleRename = async () => {
		...
		try {
			const r = await fetch(`/api/v1/depot/${encodeURIComponent(renameTarget)}`, {
				...
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
					name: renameValue.endsWith(".dxf") ? renameValue : `${renameValue}.dxf`,
				}),
			});
			if (!r.ok) {
				const j = await r.json();
	...
	const handleExport = async (format: string) => {
		...
		try {
			...
			const r = await fetch("/api/v1/control/tool", {
				...
				body: JSON.stringify({
					tool: "plan_export",
					arguments: { file_name: selectedFile, format, output_name: outName },
				}),
			});
			const j = await r.json();
			...
			<div className="flex items-center justify-between flex-wrap gap-3">
				...
				<div className="flex items-center gap-2">
					<button
						type="button"
						onClick={loadFiles}
						className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-white/10 text-sm text-slate-400 hover:text-white hover:bg-white/10"
					>
					...
					<button
						...
						className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-white text-sm font-bold"
					>
			...
			<div className="flex items-center gap-3">
				...
				<div className="flex gap-1 p-1 bg-white/10 rounded-xl">
					<button
						...
						className={`p-1.5 rounded-lg ${viewMode === "grid" ? "bg-amber-600 text-white" : "text-slate-300 hover:text-slate-300"}`}
					>
						<Grid3X3 size={14} />
					</button>
					<button
						...
						className={`p-1.5 rounded-lg ${viewMode === "list" ? "bg-amber-600 text-white" : "text-slate-300 hover:text-slate-300"}`}
					>
						<List size={14} />
					</button>
				</div>
					...
					{files.length === 0 && (
						<div className="flex gap-3 justify-center mt-4">
							...
							<button
								type="button"
								onClick={() => setShowCreate(true)}
								className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-white text-sm font-bold"
							>
					...
					<div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
						...
						<div className="flex items-center gap-2">
							...
							<a
								...
								download
								className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-bold"
							>
						...
						<div className="space-y-3">
							...
							{fileInfo?.layers && (
								<div className="bg-[#0a0a0c] border border-white/10 rounded-xl p-3 space-y-2">
									...
									{fileInfo.layers.map((l: { name: string; color: number; frozen?: boolean }) => (
							...
							<div className="flex gap-2">
								<button
									...
									onClick={() => {
										...
										setRenameValue(selectedFile.replace(/\.dxf$/i, ""));
									}}
									className="flex-1 flex items-center justify-center gap-1 px-3 py-2 rounded-xl border border-white/10 text-sm text-slate-400 hover:text-white"
								>
								...
								<button
									...
									className="flex-1 flex items-center justify-center gap-1 px-3 py-2 rounded-xl border border-red-500/20 text-sm text-red-400 hover:bg-red-500/10"
								>
