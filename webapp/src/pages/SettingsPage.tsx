  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="bg-[#1e1e26] border border-white/10 rounded-2xl p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <label className="block text-sm text-slate-400">
            ...
            <input type="number" step="0.1" min="0.5" value={wallHeight} onChange={(e) => setWallHeight(parseFloat(e.target.value) || 3)} className="mt-1 w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-slate-200 outline-none focus:border-amber-500/30" />
          </label>
          <label className="block text-sm text-slate-400">
            Default Wall Thickness (m)
            <input type="number" step="0.05" min="0.05" value={wallThickness} onChange={(e) => setWallThickness(parseFloat(e.target.value) || 0.3)} className="mt-1 w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-slate-200 outline-none focus:border-amber-500/30" />
          </label>
        </div>
      </div>
      ...
      <div className="bg-[#1e1e26] border border-white/10 rounded-2xl p-6 space-y-4">
        ...
        <label className="block text-sm text-slate-400">
          Ollama / LMStudio URL
          <input value={ollamaUrl} onChange={(e) => setOllamaUrl(e.target.value)} className="mt-1 w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-slate-200 outline-none focus:border-amber-500/30" />
        </label>
        <label className="block text-sm text-slate-400">
          Model
          <input value={model} onChange={(e) => setModel(e.target.value)} className="mt-1 w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-slate-200 outline-none focus:border-amber-500/30" />
        </label>
      </div>
      ...
      <div className="bg-[#1e1e26] border border-white/10 rounded-2xl p-6 space-y-4">
        ...
        <label className="block text-sm text-slate-400">
          ...
          <input value={qcadProPath} onChange={(e) => setQcadProPath(e.target.value)} placeholder="C:\Program Files\QCAD\qcad.exe" className="mt-1 w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-slate-200 outline-none focus:border-amber-500/30" />
        </label>
