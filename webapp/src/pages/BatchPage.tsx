...
interface PlanInfoData { entity_total: number; layer_count: number; block_count: number; dxf_version: string }
interface PlanAnalyseData { rooms: unknown[]; wall_length_m: number }
interface StlResult { stl: string; vertices: number }
...
export default function BatchPage() {
	...
	const run = async () => {
		...
		setRunning(true); setError(""); setResults(null); setStlResults([]);
		try {
		...
		for (const f of dxfFiles) {
			try {
				const r = await fetch(API_BASE + "/api/v1/control/tool", {
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({ tool: "plan_extrude", arguments: { file_name: f.name, wall_layers: ["Walls"], wall_height: 3.0, wall_thickness: 0.15 } }),
				});
				const j = await r.json();
		...
		<div className="max-w-4xl space-y-6">
			...
			{stlResults.length > 0 && (
				<div className="space-y-2">
					<p className="text-sm text-slate-400 flex items-center gap-2"><Box size={14} className="text-amber-400" /> {stlResults.length} STL files generated</p>
					<div className="grid grid-cols-2 md:grid-cols-4 gap-3">
						{stlResults.map((s, i) => (
							<div key={i} className="bg-[#1e1e26] border border-white/10 rounded-xl p-3 text-center">
								...
								<a href={`/api/v1/download/${s.stl}`} className="inline-flex items-center gap-1 mt-2 text-amber-400 text-xs hover:underline">
			...
			{results && (
				<div className="space-y-2">
					<p className="text-sm text-slate-400">{results.filter((r) => r.success).length}/{results.length} succeeded</p>
					{results.map((r) => (
						<div key={r.file} className={`bg-[#1e1e26] border rounded-xl p-3 ${r.success ? "border-emerald-500/20" : "border-red-500/20"}`}>
							...
							{r.success && tool === "plan_info" && (
								<div className="mt-2 text-xs text-slate-400 space-y-0.5">
									<span>Entities: {(r.data as PlanInfoData)?.entity_total ?? "?"} · Layers: {(r.data as PlanInfoData)?.layer_count ?? "?"} · Blocks: {(r.data as PlanInfoData)?.block_count ?? "?"} · DXF: {(r.data as PlanInfoData)?.dxf_version ?? "?"}</span>
								</div>
							)}
							{r.success && tool === "plan_analyse" && (
								<div className="mt-2 text-xs text-slate-400 space-y-0.5">
									<span>Rooms: {(r.data as PlanAnalyseData)?.rooms?.length ?? "0"} · Wall length: {(() => { const wl = (r.data as PlanAnalyseData)?.wall_length_m; return wl != null ? `${wl.toFixed(1)}m` : "?"; })()}</span>
								</div>
							)}
