...
export default function AnalysePage() {
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <p className="text-sm text-slate-300">Detect rooms from enclosed polylines, calculate areas in m², and identify doors/windows by block names.</p>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-[#1e1e26] border border-white/10 rounded-2xl p-4 text-center">
              ...
              <p className="text-2xl font-bold text-white">{result.wall_length_m}<span className="text-sm font-normal text-slate-300"> m</span></p>
              <p className="text-sm text-slate-300">Total Wall Length</p>
            </div>
            <div className="bg-[#1e1e26] border border-white/10 rounded-2xl p-4 text-center">
              <BarChart3 size={20} className="mx-auto mb-1 text-amber-400" />
              <p className="text-2xl font-bold text-white">{result.rooms?.length || 0}</p>
              <p className="text-sm text-slate-300">Rooms / Spaces</p>
            </div>
            <div className="bg-[#1e1e26] border border-white/10 rounded-2xl p-4 text-center">
              ...
              <p className="text-sm text-slate-300">Doors / Windows</p>
            </div>
          </div>
                    ...
                    {result.rooms.map((room: any, i: number) => (
                      <tr key={i} className="border-b border-white/[0.02] text-slate-300">
                        ...
                        <td className="py-2 pr-4">
                          <span className={`px-2 py-0.5 rounded text-sm ${room.likely_type === "wall_outline" ? "bg-red-950/30 text-red-400" : "bg-emerald-950/30 text-emerald-400"}`}>
                            {room.likely_type}
                          </span>
                        </td>
            ...
            <div className="bg-[#1e1e26] border border-white/10 rounded-2xl p-4 space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {result.doors_windows.map((item: any, i: number) => (
                  <div key={i} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/10 text-sm">
                    <DoorOpen size={12} className="text-amber-400" />
                    <span className="text-slate-300">{item.block}</span>
                    <span className="text-slate-400">on {item.layer}</span>
                    <span className="text-slate-300 text-sm">({item.position.x.toFixed(0)}, {item.position.y.toFixed(0)})</span>
                  </div>
                ))}
              </div>
            </div>
