...
export default function AppLayout({ children }: { children: React.ReactNode }) {
	...
	const statusColor =
		state === "connected" ? "text-emerald-400" : state === "connecting" ? "text-amber-400" : "text-red-400";
				...
				<header className="h-12 flex items-center justify-between px-6 border-b border-white/10 bg-[#0a0a0c] shrink-0">
					...
					<div className="flex items-center gap-3">
						...
						<button
							...
							onClick={() => {
								try {
									window.open("", "", "width=1100,height=750");
								} catch {}
							}}
						...
						<div className="flex items-center gap-2 text-sm">
							...
							<span data-testid="connection-label" className={statusColor}>
								{statusLabel}
							</span>
							{state !== "connected" && (
								<button
