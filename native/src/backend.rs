...
fn materialize_backend(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    let cache = app.path().app_cache_dir().map_err(|e| format!("cache dir: {e}"))?;
    fs::create_dir_all(&cache).map_err(|e| format!("create cache: {e}"))?;
    let cached = cache.join(BACKEND_NAME);
    fs::copy(&bundled, &cached).map_err(|e| format!("copy to cache: {e}"))?;
    log_line(app, &format!("materialized backend to cache: {}", cached.display()));
    Ok(cached)
}
...
fn free_port(port: u16) -> bool {
    ...
    {
        let img_kill = format!(
            "Stop-Process -Name 'qcad-mcp-backend' -Force -ErrorAction SilentlyContinue; \
             Stop-Process -Name 'qcad-mcp-native' -Force -ErrorAction SilentlyContinue; \
             taskkill /F /IM qcad-mcp-backend.exe /T 2>$null; \
             taskkill /F /IM qcad-mcp-native.exe /T 2>$null"
        );
            ...
            if i == 15 {
                let elevated = format!(
                    "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                     '-NoProfile -Command \"Stop-Process -Name qcad-mcp-backend -Force -ErrorAction SilentlyContinue; \
                     taskkill /F /IM qcad-mcp-backend.exe /T 2>$null; \
                     Get-NetTCPConnection -LocalPort {port} -ErrorAction SilentlyContinue | \
                     ForEach-Object {{ taskkill /F /PID $_.OwningProcess /T 2>$null }}\"'"
                );
